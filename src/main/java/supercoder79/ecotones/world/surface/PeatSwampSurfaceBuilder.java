...
public class PeatSwampSurfaceBuilder extends SurfaceBuilder<TernarySurfaceConfig> {
    ...
    public void generate(Random random, Chunk chunk, Biome biome, int x, int z, int height, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, TernarySurfaceConfig surfaceBlocks) {
        if (noise > 0.3 && noise < 1.2) {
            SurfaceBuilder.DEFAULT.generate(random, chunk, biome, x, z, height, noise, defaultBlock, defaultFluid, seaLevel, seed, new TernarySurfaceConfig(EcotonesBlocks.PEAT_BLOCK.getDefaultState(), EcotonesBlocks.PEAT_BLOCK.getDefaultState(), EcotonesBlocks.PEAT_BLOCK.getDefaultState()));
        } else {
