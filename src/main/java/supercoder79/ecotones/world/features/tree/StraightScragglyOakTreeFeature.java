...
public class StraightScragglyOakTreeFeature extends EcotonesFeature<OakTreeFeatureConfig> {
    public StraightScragglyOakTreeFeature(Codec<OakTreeFeatureConfig> configCodec) {
        super(configCodec);
    }
    ...
    public boolean generate(FeatureContext<OakTreeFeatureConfig> context) {
        ...
        generateLeaves(world, random, leafNodes);
    ...
    private void generateLeaves(StructureWorldAccess world, Random random, List<BlockPos> leafNodes) {
        for (BlockPos node : leafNodes) {

            int yMin = random.nextInt(2);
            int yMax = random.nextInt(2);
            yMax = (yMin | yMax) == 0 ? 1 : yMax;

            generateSmallLeafLayer(world, node.up(yMax + 1));
            generateSmallLeafLayer(world, node.down(yMin + 1));
