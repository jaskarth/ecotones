...
public class PatchFeatureConfig implements FeatureConfig {
    public static final Codec<PatchFeatureConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            UniformIntDistribution.CODEC.fieldOf("radius").forGetter(config -> config.radius)
    ...
    private PatchFeatureConfig(BlockState state, Identifier target, UniformIntDistribution radius) {
        this(state, Registry.BLOCK.get(target), radius);
    }

    public PatchFeatureConfig(BlockState state, Block target, UniformIntDistribution radius) {
        this.state = state;
        this.target = target;
        this.radius = radius;
    }
