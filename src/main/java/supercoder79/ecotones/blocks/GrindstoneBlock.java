...
import supercoder79.ecotones.blocks.entity.GrindstoneBlockEntity;
...
public class GrindstoneBlock extends BlockWithEntity {
    ...
    protected GrindstoneBlock(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            ...
            if (blockEntity instanceof GrindstoneBlockEntity grindstone) {
                boolean interacted = false;

                if (!interacted) {
                    player.openHandledScreen(grindstone);
                }
            }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, EcotonesBlockEntities.GRINDSTONE, GrindstoneBlockEntity::tick);
    }
}
